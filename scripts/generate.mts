...
function filesSection(hasBlock: boolean): string {
  ...
  if (hasBlock) rows.push('| [`assets/block.svg`](assets/block.svg) / [`.png`](assets/block.png) | Compact one-block explainer view. |');
  return `## Files\n\n${rows.join('\n')}`;
}

function renderSpecReadme(s: DecoderSpec | EncoderSpec, model: Graph, check: ParamCheck): string {
...
${fullGraphSection(s.id, s.displayName, model, true)}
...
function renderHFFullReadme(e: HFFullEntry, model: Graph, check: ParamCheck): string {
...
${fullGraphSection(e.id, e.displayName, model, !!e.blockTemplate)}
...
for (const s of ALL) {
  if (s.kind === 'decoder' || s.kind === 'encoder') {
    ...
    const blockView = s.kind === 'decoder' ? buildDecoderBlockView(s) : buildEncoderBlockView(s);
    wireGraph(blockView); validateShapes(blockView);
    writeAssets(s.id, model, blockView);
    fs.writeFileSync(path.join(ZOO, 'architectures', s.id, 'README.md'), renderSpecReadme(s, model, check));
    console.log(`✓ ${s.id} [${s.kind}] ${model.components.length} comps | est ${fmtP(check.est)}${check.expected ? ` vs ${fmtP(check.expected)} (${check.devPct!.toFixed(1)}%)` : ''} | ${warnings} warnings`);
  } else if (s.kind === 'hffull') {
    const desc = `${s.displayName} (${s.org}): full graph; dimensions verified against the official config.json.`;
    const model = await importFullGraph(s, s.id, s.displayName, desc);
    const warnings = validateShapes(model);
    const check = paramGate(s.id, model, s);
    let blockView: Graph | undefined;
    if (s.blockTemplate) {
      blockView = JSON.parse(fs.readFileSync(path.join(NEURARCH, 'public/templates', s.blockTemplate, 'model.json'), 'utf8'));
      wireGraph(blockView!);
    }
    writeAssets(s.id, model, blockView);
    fs.writeFileSync(path.join(ZOO, 'architectures', s.id, 'README.md'), renderHFFullReadme(s, model, check));
    console.log(`✓ ${s.id} [hffull] ${model.components.length} comps | est ${fmtP(check.est)}${check.expected ? ` vs ${fmtP(check.expected)} (${check.devPct!.toFixed(1)}%)` : ''} | ${warnings} warnings`);
