...
沿用论文的通用画法,不用对照图例也能读懂:

| 记号 | 含义 |
|------|------|
| **⊕** | 逐元素**相加**,把两个同形张量求和。这就是**残差 / 跳跃连接**(`y = x + F(x)`);绕过某个块的那条曲线就是 skip。 |
| **∥** | **拼接 (concatenate)**,把两条流沿某个维度首尾接成更宽的张量(如 DenseNet、NCF、DCN)。拼接维变长,其余维不变。 |
| **⊗** | 逐元素**相乘 / 门控**,一条流缩放另一条流。 |
| `similarity`、`cross_att`、`matmul` | 两条流汇合处的具名算子。**matmul / similarity** 是点积(相乘**再求和**,是收缩 contraction,不是相加),如 CLIP 的图文相似度;**cross-attention** 是一条流的 query 去注意另一条流的 key/value(编码器-解码器、多模态 VLM)。 |
| **`× N` 角标** | 一串 **N 个相同的块被折叠成一个**,方便一屏放下。`model.json` 始终保留全部 N 个、真实维度。 |
| **并排的列** | 各自独立的**分支 / 塔 (towers)**,分别计算,只在箭头汇合处合并(如 CLIP 的图像塔 + 文本塔 → `similarity`)。 |

在 Neurarch 画布里左下角的 **Legend** 会实时显示同样的图例,只列出当前模型实际用到的记号。
...
只隔离一个设计选择的姊妹块,所以图上的 diff 恰好就是那个机制。两组并排对照见 [COMPARISONS.md](COMPARISONS.md#attention-sparsity-full--sliding-window--sparse)。

| 架构 | 机制 | 亮点 |
|--------------|-----------|---------|
| [attn-full](architectures/attn-full/) | 全注意力 | O(n²) 稠密基线(Vaswani 等) |
| [attn-sliding-window](architectures/attn-sliding-window/) | 滑动窗口注意力 | 线性成本的局部窗口(Longformer、Mistral) |
| [attn-sparse](architectures/attn-sparse/) | 块稀疏注意力 | top-k 块,硬件对齐(DeepSeek NSA) |
| [posenc-learned](architectures/posenc-learned/) | 学习式绝对位置 | 加到 embedding 的可训练表(GPT、BERT) |
| [posenc-rope](architectures/posenc-rope/) | 旋转位置(RoPE) | 相对位置,可外推(Llama、Qwen) |
| [posenc-alibi](architectures/posenc-alibi/) | 线性偏置位置(ALiBi) | 无 embedding,距离惩罚(MPT、BLOOM) |
...
| [falcon-7b](architectures/falcon-7b/) | TII | 7.2B | 多查询注意力(1 个 KV 头)+ 并行 attn/MLP 残差 |
| [mpt-7b](architectures/mpt-7b/) | MosaicML | 6.6B | ALiBi 位置(无位置嵌入),可外推长度;tied 嵌入 |
| [jamba](architectures/jamba/) | AI21 | 52B / 12B | 混合架构:Mamba(SSM)+ 注意力 + MoE,7:1 SSM:注意力 |
